...
export default function AvatarSurvey({ survey }: AvatarSurveyProps) {
  ...
  const [avatarEmotion, setAvatarEmotion] = useState<'happy' | 'neutral' | 'excited' | 'concerned'>('happy');
    ...
    const welcomeMessages: Message[] = [
      {
        ...
        role: 'assistant',
        content: `Hi! 👋 I'm Aiko, your personal health assistant!`,
        timestamp: new Date(),
      },
      {
        id: generateMessageId('welcome'),
        role: 'assistant',
        content: "I'd love to learn about how you're feeling today. This will only take a few minutes!",
        timestamp: new Date(),
      },
    ];
    ...
    if (llmResponse.parsedValue !== null) {
      ...
      if (state.currentQuestionIndex < survey.questions.length - 1) {
        ...
        const completionMessage: Message = {
          ...
          role: 'assistant',
          content: "🎉 Thank you so much for completing the survey! Your responses have been recorded.",
          timestamp: new Date(),
        };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50 relative overflow-hidden">
      ...
      <div className="relative z-10 h-screen flex max-w-7xl mx-auto">
        {/* Left side - Chat interface */}
        <div className="flex-1 flex flex-col">
          {/* Header */}
          <motion.div
            className="bg-white/80 backdrop-blur-md shadow-md px-6 py-4 border-b border-gray-200"
          ...
          {state.isComplete && (
            ...
            >
              <button
                ...
                className="w-full px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition-shadow"
              >
        ...
        <motion.div
          className="w-96 bg-gradient-to-br from-purple-100/50 to-pink-100/50 backdrop-blur-sm border-l border-white/20 flex items-center justify-center"
          initial={{ x: 100, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
          transition={{ duration: 0.8, delay: 0.3 }}
        >
          <AnimatedAvatar isThinking={isProcessing} emotion={avatarEmotion} />
        </motion.div>
      </div>
    </div>
  );
